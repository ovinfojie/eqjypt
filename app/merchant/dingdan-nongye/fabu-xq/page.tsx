...
export default function FabuXqPage() {
  ...
  const [priceMin, setPriceMin] = useState("2.70")
  const [priceMax, setPriceMax] = useState("3.20")
    ...
    <div>
      ...
      <div className="flex gap-5">
        <div className="flex-1 bg-white rounded border border-[#e8edf5] p-6">
          ...
          <div className="mt-4">
            <Field label="需求标题" required><Input placeholder="请输入" /></Field>
              ...
              <div className="flex items-center gap-6 h-9">
                {(["public", "directed"] as const).map(t => (
                  <label key={t} className="flex items-center gap-1.5 cursor-pointer text-[13px]">
                    <input type="radio" checked={publishType === t} onChange={() => setPublishType(t)} className="accent-[#1a5fa8]" />
                    {t === "public" ? "公开发布" : "定向发布"}
                  </label>
                ))}
              </div>
            ...
            {publishType === "directed" && (
              <Field label="指定供应商" required><Input placeholder="请输入" /></Field>
            )}
            ...
            <div className="mb-4">
              <div className="flex gap-3">
                {[
                  { key: "desc", label: "需求描述", sub: "直接描述需求，适合需求较灵活或尚未确定具体商品的情况" },
                  { key: "product", label: "选择商品", sub: "关联已有商品档案，适合已有明确商品规格的需求" },
                ].map(opt => (
                  <button key={opt.key} onClick={() => setContentType(opt.key as "desc" | "product")}
                    className={`flex-1 p-3 rounded border-2 text-left transition-colors ${contentType === opt.key ? "border-[#1a5fa8] bg-[#f0f7ff]" : "border-[#e8edf5] bg-white hover:border-[#c5d8f0]"}`}>
                    <div className="flex items-center gap-2 mb-1">
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 ${contentType === opt.key ? "border-[#1a5fa8]" : "border-[#bbb]"}`}>
                ...
                <table className="w-full text-[13px]">
                  <thead>
                    <tr className="bg-[#f5f7fa] border-b border-[#e8edf5]">
                      ...
                      <th className="text-left px-4 py-2.5 text-[#555] font-medium w-48">计划采购量（单位）</th>
                      <th className="text-left px-4 py-2.5 text-[#555] font-medium w-52">收购价区间（单位）</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="bg-white">
                      ...
                      <td className="px-4 py-3">
                        <div className="relative">
                          <button
                            ...
                            className="flex items-center justify-between gap-1 border border-[#dde3ec] rounded px-3 h-8 text-[13px] w-36 focus:outline-none focus:border-[#1a5fa8] bg-white hover:border-[#1a5fa8] transition-colors"
                          >
                          ...
                          {showProductDropdown && (
                            <div className="absolute top-9 left-0 w-48 bg-white border border-[#e8edf5] rounded shadow-lg z-10">
                              {MOCK_PRODUCTS.map(p => (
                                <button key={p.id} onClick={() => { setSelectedProduct(p); setShowProductDropdown(false) }}
                                  className={`w-full text-left px-3 py-2 text-[13px] hover:bg-[#f0f7ff] transition-colors ${selectedProduct.id === p.id ? "text-[#1a5fa8] bg-[#f0f7ff]" : "text-[#333]"}`}>
                                  {p.name}
                                </button>
                              ))}
                            </div>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          ...
                          <button className="text-[#1a5fa8] text-[11px] hover:underline"><ChevronDown className="w-3.5 h-3.5" /></button>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <input value={qty} onChange={e => setQty(e.target.value)}
                            className="border border-[#dde3ec] rounded px-2 h-7 text-[13px] w-20 focus:outline-none focus:border-[#1a5fa8] text-right" />
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <input value={priceMin} onChange={e => setPriceMin(e.target.value)}
                            className="border border-[#dde3ec] rounded px-2 h-7 text-[13px] w-16 focus:outline-none focus:border-[#1a5fa8] text-right" />
                ...
                <div className="flex items-center gap-6 h-9">
                  {[["editable", "可以修改报价"], ["fixed", "一次报价不可修改"]].map(([val, label]) => (
                    <label key={val} className="flex items-center gap-1.5 cursor-pointer text-[13px]">
                      <input type="radio" checked={quoteMode === val} onChange={() => setQuoteMode(val as "editable" | "fixed")} className="accent-[#1a5fa8]" />
                      {label}
                    </label>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-4 h-9 flex-wrap">
                  {["担保交易", "非担保交易"].map(v => (
                    <label key={v} className="flex items-center gap-1.5 cursor-pointer text-[13px]">
                      <input type="checkbox" checked={tradeMode.includes(v)} onChange={() => toggleArr(tradeMode, v, setTradeMode)} className="accent-[#1a5fa8]" />
                      {v}
                    </label>
                  ))}
                </div>
              ...
              <Field label="配送方式" required>
                <div className="flex items-center gap-4 h-9 flex-wrap">
                  {["卖家配送", "买家自提", "无需物流"].map(v => (
                    <label key={v} className="flex items-center gap-1.5 cursor-pointer text-[13px]">
                      <input type="checkbox" checked={deliveryMethod.includes(v)} onChange={() => toggleArr(deliveryMethod, v, setDeliveryMethod)} className="accent-[#1a5fa8]" />
                      {v}
                    </label>
                  ))}
                </div>
            ...
            <Field label="结算渠道" required>
              <div className="flex items-center gap-4 h-9 flex-wrap">
                {["建行龙存管", "工行安心付"].map(v => (
                  <label key={v} className="flex items-center gap-1.5 cursor-pointer text-[13px]">
                    <input type="checkbox" checked={settlement.includes(v)} onChange={() => toggleArr(settlement, v, setSettlement)} className="accent-[#1a5fa8]" />
                    {v}
                  </label>
                ))}
              </div>
        ...
        <div className="w-52 shrink-0">
          <div className="bg-white rounded border border-[#e8edf5] p-4 sticky top-4">
            ...
            <div className="space-y-2 text-[12px] text-[#666]">
              {["需求标题", "发布方式", "指定供应商", contentType === "product" ? "选择商品" : "需求描述", "期望收货时间", "预付款比例", "报价模式", "交易模式", "结算渠道"].map(f => (
